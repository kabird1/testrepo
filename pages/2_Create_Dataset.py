...
def load_new_image():
    if st.session_state.counter<len(st.session_state.data.latitude):
        ...
        if map.ok:
            ...
            st.session_state.col1.image_container.image(image=display_image, caption="Satellite image at coordinates latitude="+str(latitude)+", longitude="+str(longitude)+", Copyright Map data ©2023")
...
if st.session_state.user_file!=None:
    ...
    if len(st.session_state.data.latitude)>0:
        st.session_state.col1, col2 = st.columns(2)
        load_new_image()
        col21, col22, col23= col2.columns(3)
        col21.button(label="yes", help="yes = The feature IS shown in the image", on_click=yes_button_callback, use_container_width=True)
        col22.button(label='No', help="No = The feature IS NOT shown in the image", on_click=no_button_callback, use_container_width=True)
        col23.button(label="Inconclusive", help = "Inconclusive = Unsure if feature is shown in the image", on_click=inc_button_callback, use_container_width=True)
